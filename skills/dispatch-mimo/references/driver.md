# Mimo driver — §5a, §5c, §6a, §6c, §6d, §6g, §6h

Everything in `dispatch-mimo` that depends on codex itself. A mimo lane is the codex CLI
(`--kind codex`) with its model pinned to `Mimo/mimo-v2.6-pro` and its reasoning effort pinned to `high`, served on its
route to Mimo (xiaomi) through
the opencodex proxy (`openai_base_url` in `~/.codex/config.toml`). Every codex mechanism below —
launch flags, rollout JSONL, goals sqlite, trust dialog — is the CLI's own and is model-agnostic.
The agent-independent halves are `references/plan.md` (§2–§4, §5b) and `references/supervise.md`
(§6b, §6e, §6f, §6i, §7, §8); §0 and §1 are in `../SKILL.md`.

Claims marked **(verified)** were checked against codex **0.155.1** with the Mimo model above on
this machine — including headless `codex exec -c model=…` round-trips through the proxy. The rollout records `model_context_window: 121600` for the slug — the catalog's 95% effective
share of its 128000 window — which is what §6a's `used_pct` divides by. §2 records the version it actually finds; a mismatch is a report line, not an abort.
...
- `codex` not found, or the version refuses to resolve → stop that lane and report it (for the asdf
  ...
  redirect it.
- Mimo route reachable (mimo only; both checks are local and cost no tokens): (a) the opencodex proxy
  answers — extract `openai_base_url` from `~/.codex/config.toml` and poll `<base-url>/models`;
  connection refused means opencodex is not running. (b) the slug exists — `jq` the
  `model_catalog_json` file for `Mimo/mimo-v2.6-pro`. Either failing → stop the
  run and tell the user to start opencodex or fix its catalog; a lane launched without the proxy
  dies on its first turn, after the worktree already exists. Catalog presence alone does not prove
  the route serves traffic — §5c's effort trap is the worked example: when a lane's every turn
  dies with `Provider error 400: Invalid request parameters`, stop the run and surface it rather
  than relaunching.
...
## §5c Launch mimo and set the goal
...
- `-c 'model="Mimo/mimo-v2.6-pro"'` pins the model at launch — with the effort pin below, this is the whole
  difference from dispatch-codex. The TUI `/model` picker is an interactive selection list that
  would need dialog handling after launch; a `-c` override takes effect before the first paint,
  and it cannot drift when the user later changes `model` in `~/.codex/config.toml`. Verified: a
  fresh codex resolves the slug through the opencodex proxy with no interactive step.
- `-c 'model_reasoning_effort="high"'` pins the reasoning effort, and `high` is a measured
  ceiling, not a preference: verified against this route on 2026-09-22, `medium` and `high`
  round-trips succeed while `xhigh`, `max` and `ultra` — all declared in the catalog's
  `supported_reasoning_levels` — are rejected by the upstream on every send with `Provider error
  400: Invalid request parameters`. Do not raise the pin without a fresh headless round-trip. The
  pin also guards against a global `model_reasoning_effort` in `~/.codex/config.toml` (verified
  trap: a machine-wide `max` silently rides along on every unpinned launch and kills the lane's
  first turn).
...
- rate limits — all lanes share one Mimo (xiaomi) account behind the opencodex proxy,
  so its limits can throttle every lane at once (surfacing as `goal_status: usage_limited`, §6d).
  The window is also the smallest of the pinned-model dispatchers — 128K
  (`model_context_window: 121600`), so §6c's 70% compaction threshold arrives near ~85K used:
  expect compactions, and §6h's 3-compaction restart handoff, sooner than on a 1M-window lane.
...
If `~/.claude/dispatch-mimo/bin/lane_state.py` is absent, write it, then call
`python3 ~/.claude/dispatch-mimo/bin/lane_state.py <session-uuid>`. One python3 call instead of a
...
- `usage_limited` — the shared Mimo (xiaomi) account hit its rate window (§3 warned
  about this). Resume
