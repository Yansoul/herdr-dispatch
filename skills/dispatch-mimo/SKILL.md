---
name: dispatch-mimo
description: Dispatch one or more tasks to mimo agents (the codex CLI with its model pinned to Mimo/mimo-v2.6-pro) in herdr workspaces — the orchestrator designs each lane's plan and acceptance criteria, mimo pursues them in goal mode — supervise them to completion, then push, independently review, and merge each lane's pull request. Use when the user asks to dispatch/fan out/派发 tasks to mimo agents or herdr lanes, to run several mimo tasks in parallel worktrees, or to resume supervision of an existing dispatch-mimo run (`--resume`).
...
# Dispatch tasks to mimo agents running under herdr
...
lanes, give each lane its own herdr workspace and its own mimo agent, and then keep those agents
alive and moving until every lane genuinely finishes. **You do the planning; mimo does the
...
when its work is verified, its branch is pushed, and its pull request is open, independently reviewed, and merged (§6f, §6j).

Each mimo lane is the codex CLI (`--kind codex`) launched with its model pinned to
`Mimo/mimo-v2.6-pro` and its reasoning effort pinned to `high` (§5c) through the opencodex proxy. "codex" below names the CLI
and its mechanics — launch flags, rollout JSONL, goals sqlite — all of which are model-agnostic;
the two pins — model and reasoning effort — are the only things this dispatcher changes.
...
1. **The state file is the only truth.** `~/.claude/dispatch-mimo/<run-id>/state.json`. Begin every
...
`~/.claude/dispatch-mimo/*/state.json` for runs whose repo matches the cwd and that still hold
