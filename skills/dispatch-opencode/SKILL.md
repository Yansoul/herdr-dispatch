...
description: Dispatch one or more tasks to opencode agents in herdr workspaces — the orchestrator designs each lane's plan and acceptance criteria, opencode executes them turn by turn under the supervision loop — then push, independently review, and merge each lane's pull request. Use when the user asks to dispatch/fan out/派发 tasks to opencode agents or herdr lanes, to run several opencode tasks in parallel worktrees, or to resume supervision of an existing dispatch-opencode run (`--resume`).
argument-hint: '<task-1>; <task-2>; … [--lanes N] [--base <ref>] [--no-yolo] [--compact-at N] [--draft] [--no-pr] [--no-automerge] [--resume] [--no-loop]'
