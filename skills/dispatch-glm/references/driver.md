# GLM driver — §5a, §5c, §6a, §6c, §6d, §6g, §6h

Everything in `dispatch-glm` that depends on codex itself. A glm lane is the codex CLI
(`--kind codex`) with its model pinned to `zhipu-bigmodel-coding/glm-5.3-flash`, served through
...
Claims marked **(verified)** were checked against codex **0.155.1** with the GLM model above on
this machine — including a headless `codex exec -c model=…` round-trip through the proxy and the
catalog entry publishing `context_window: 1000000` for the slug, which is what §6a's `used_pct`
divides by. §2 records the version it actually finds; a mismatch is a report line, not an abort.
...
- `codex` not found, or the version refuses to resolve → stop that lane and report it (for the asdf
  ...
  redirect it.
- GLM route reachable (glm only; both checks are local and cost no tokens): (a) the opencodex proxy
  ...
  `model_catalog_json` file for `zhipu-bigmodel-coding/glm-5.3-flash`. Either failing → stop the
  run and tell the user to start opencodex or fix its catalog; a lane launched without the proxy
  dies on its first turn, after the worktree already exists.
...
## §5c Launch glm and set the goal
...
so nothing is sent to a pane whose composer you have not seen:
    ...
    herdr agent start <lane> --kind codex --pane <pane> --timeout 120000 -- \
      ...
      -c 'model="zhipu-bigmodel-coding/glm-5.3-flash"' \
...
- `-c 'model="zhipu-bigmodel-coding/glm-5.3-flash"'` pins the model at launch — this is the whole
...
- rate limits — all lanes share one GLM (zhipu-bigmodel-coding) account behind the opencodex proxy,
...
If `~/.claude/dispatch-glm/bin/lane_state.py` is absent, write it, then call
`python3 ~/.claude/dispatch-glm/bin/lane_state.py <session-uuid>`. One python3 call instead of a
...
- `usage_limited` — the shared GLM (zhipu-bigmodel-coding) account hit its rate window (§3 warned
  about this). Resume
