---
name: dispatch-glm
description: Dispatch one or more tasks to glm agents (the codex CLI with its model pinned to zhipu-bigmodel-coding/glm-5.3-flash) in herdr workspaces — the orchestrator designs each lane's plan and acceptance criteria, glm pursues them in goal mode — supervise them to completion, then push, independently review, and merge each lane's pull request. Use when the user asks to dispatch/fan out/派发 tasks to glm agents or herdr lanes, to run several glm tasks in parallel worktrees, or to resume supervision of an existing dispatch-glm run (`--resume`).
...
# Dispatch tasks to glm agents running under herdr
...
lanes, give each lane its own herdr workspace and its own glm agent, and then keep those agents
alive and moving until every lane genuinely finishes. **You do the planning; glm does the
...
Each glm lane is the codex CLI (`--kind codex`) launched with its model pinned to
`zhipu-bigmodel-coding/glm-5.3-flash` through the opencodex proxy. "codex" below names the CLI
and its mechanics — launch flags, rollout JSONL, goals sqlite — all of which are model-agnostic;
the pinned model is the only thing this dispatcher changes.
...
1. **The state file is the only truth.** `~/.claude/dispatch-glm/<run-id>/state.json`. Begin every
...
`~/.claude/dispatch-glm/*/state.json` for runs whose repo matches the cwd and that still hold
